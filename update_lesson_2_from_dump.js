...
async function main() {
  ...
  const dumpPath = path.join(__dirname, 'lesson2_dump.txt');
  
  try {
    const content = fs.readFileSync(dumpPath, 'utf-8');
    if (!content.trim()) {
      console.log('File is empty. Please paste the content into lesson2_dump.txt');
      return;
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Codes for lesson 2 start with '2-' and are followed by digits
      const normalizedLine = line.replace(/\s+/g, '');
      
      if (normalizedLine.match(/^2-\d+$/)) {
    ...
    // Get Lesson 2 ID
    const { data: lessons } = await supabase.from('lessons').select('id').eq('index', 2).single();
    if (!lessons) {
      console.error('Lesson 2 not found! Please create it first.');
      return;
    }
    const lessonId = lessons.id;

    // Delete existing lines for this lesson
    console.log('Deleting existing lines for Lesson 2...');
