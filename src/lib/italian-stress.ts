...
const ACCENTED_VOWELS = ['á', 'é', 'í', 'ó', 'ú', 'à', 'è', 'ì', 'ò', 'ù'];
...
function applyAutomaticStress(text: string): StressedSegment[] {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (accentIndex !== -1) {
      ...
      while (syllableStart > 0 && /[aeiouàèìòù]/i.test(word[syllableStart - 1])) {
        syllableStart--;
      }
      while (syllableEnd < word.length && /[aeiouàèìòù]/i.test(word[syllableEnd])) {
        syllableEnd++;
      }
