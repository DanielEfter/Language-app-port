...
async function main() {
  ...
  try {
    const content = fs.readFileSync('lesson6_dump.txt', 'utf-8');
    if (!content.trim()) {
      console.log('File is empty. Please paste the content into lesson6_dump.txt');
      return;
    }
    ...
    for (const line of lines) {
      ...
      // Normalize code (remove spaces, ensure format 6-XXXXX)
    ...
    // Get Lesson 6 ID
    const { data: lessons } = await supabase.from('lessons').select('id').eq('index', 6).single();
    if (!lessons) {
      console.error('Lesson 6 not found! Please create it first.');
      return;
    }
